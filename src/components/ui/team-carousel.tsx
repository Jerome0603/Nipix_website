...
import { cn } from './utils'; // Assuming this utility correctly merges class names
...
export const TeamCarousel: React.FC<TeamCarouselProps> = ({
  ...
  titleClassName,
  infoPosition = "bottom",
  infoTextColor = "#007DFF",
  Textcolor = "#000000",
  infoBackground = "glassy",
  onMemberChange,
  onCardClick,
  initialIndex = 0,
}) => {
  ...
  useEffect(() => {
    ...
    const handleMouseEnter = () => {
      if (pauseOnHover && autoPlayEnabled && autoPlay > 0) clearInterval(interval);
    };

    const handleMouseLeave = () => {
      if (pauseOnHover && autoPlayEnabled && autoPlay > 0) {
        interval = setInterval(() => {
          paginate(1);
        }, autoPlay);
      }
    ...
    if (carouselContainer && pauseOnHover && autoPlayEnabled && autoPlay > 0) {
  ...
  }, [autoPlay, autoPlayEnabled, paginate, pauseOnHover]);
    ...
    >
      ...
      >
        ...
        {showArrows && (
          <>
            <motion.button
              onClick={() => paginate(-1)}
              className="absolute left-5 top-1/2 transform -translate-y-1/2 bg-black/60 hover:bg-black/80 text-black w-10 h-10 rounded-full flex items-center justify-center z-20 transition-all duration-300 hover:scale-110"
              whileTap={{ scale: 0.9 }}
            >
            ...
            <motion.button
              onClick={() => paginate(1)}
              className="absolute right-5 top-1/2 transform -translate-y-1/2 bg-black/60 hover:bg-black/80 text-black w-10 h-10 rounded-full flex items-center justify-center z-20 transition-all duration-300 hover:scale-110"
              whileTap={{ scale: 0.9 }}
            >
      ...
      {infoPosition === 'bottom' && members[currentIndex] && (
        ...
        >
          ...
          <p
            className="text-xl font-bold opacity-80 uppercase tracking-wider"
